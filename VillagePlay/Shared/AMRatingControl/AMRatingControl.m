...
- (void)drawRect:(CGRect)rect
{
	...
	for (int i = 0; i < _rating; i++)
	{
		if (_solidImage)
        {
            ...
            currPoint.x += (self.fontSize+3);
        }
		else
        {
            CGContextSetFillColorWithColor(UIGraphicsGetCurrentContext(), _solidColor.CGColor);
            [kDefaultSolidChar drawAtPoint:currPoint withFont:[UIFont boldSystemFontOfSize:self.fontSize]];
            currPoint.x += self.fontSize;
        }
	}
	...
	for (int i = 0; i < remaining; i++)
	{
		...
		else
        {
			...
			[kDefaultEmptyChar drawAtPoint:currPoint withFont:[UIFont boldSystemFontOfSize:self.fontSize]];
            currPoint.x += self.fontSize;
        }
	}
}
          ...
          andMaxRating:(NSInteger)maxRating
{
                                              ...
                                              kFontSize)])
	{
        ...
        _emptyColor = [UIColor grayColor];
        _solidColor = [UIColor orangeColor];
